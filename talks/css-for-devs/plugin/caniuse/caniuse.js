...
var canIUse = (function () {
    ...
    */
    ...
    //  Customise HTML here
    var TMPL_TITLE = '<h1>{title}</h1>', // feature title {title}
        TMPL_STATUS = '<p class="status">{status}</p>', // feature status (W3C Recommendation) {status}
        TMPL_DESCRIPTION = '<p>Supported from the following versions:</p>', // description to user
        TMPL_DESKTOP_TITLE = '<h2>Desktop</h2>', // desktop header
    ...
    function generateResults(feature, showOnlyNoPrefix) {
        var agents = {},
            results = {},
            currentBrowser = '',
            support = {};
    ...
    function generate(elementId) {
        ...
        for (i = 0; i < l; i++) {
            ...
            showPrefixed = $instance.getAttribute('data-show-prefixed') || 'true';
            showPrefixed = showPrefixed === 'true' ? true : false;
            feature = getFeature(featureCode);
            if (feature) {
                result = generateResults(feature);
                result.featureCode = featureCode;
                result.agents = result.agents.filter(function(item) {
                  return showPrefixed ? true : !item.prefixed;
                });
                console.log(result);
