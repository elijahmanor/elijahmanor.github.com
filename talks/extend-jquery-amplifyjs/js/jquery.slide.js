...
$.widget( "crosslink.slide", {
	...
	executeJs: function( code ) {
		...
		evalCode = this.code.replace(/((?:\$|appendTo|insertBefore|insertAfter|prependTo|jQuery)\(\s?(?:'|"))/g,
					"$1#" + id + demoArea + " " );
		eval( evalCode );
	...
	_prepJsCode: function( elem ) {
		...
		if ( this.$preJs.is( ".javascript" ) ) {
			this.code = this.$preJs.text().replace( /\n/gm, "<br/>" );
			this.$preJs.snippet( "javascript", { style: this.options.codeTheme, showNum: true, menu: false } );
...
$.widget( "crosslink.slidemanager", {
	...
	_slide: function( slideNum, direction, forceSlide) {
		...
		if( transition.modernizrCheck() ) {
			...
			transition.transitionShim.call( this, slideNum, direction );
		}
